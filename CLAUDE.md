...
<ctx-optimize>
  ...
  <store>Pre-built knowledge store at `~/ctxoptimize/declarativex/` (config in `.ctxoptimize/` here).</store>
  ...
  <no-local-store>Fresh clone with nothing at `~/ctxoptimize/declarativex/`? Run `ctx-optimize up` —
